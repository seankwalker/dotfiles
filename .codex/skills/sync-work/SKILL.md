---
name: sync-work
description: Reconcile progress from independent Codex worker sessions into today's Obsidian daily note, then show what is active, ready for review, blocked, or complete.
---

# Sync Work

Keep the driver session responsive. Read compact deltas from independent top-level sessions; do not spawn subagents or wait for active workers.
...
3. Add a newly discovered follow-up, decision, or blocker under `## Captured` only when it is actionable and not already present.
...
Return a compact ledger grouped by Active, Ready for review, Blocked, Complete, and Unclear; omit empty groups. End with the one or two decisions that deserve the user's attention. Report the note path when it changed.
